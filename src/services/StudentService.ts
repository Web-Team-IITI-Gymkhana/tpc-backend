...
import { COMPANY_DAO, PENALTY_DAO, STUDENT_DAO, USER_DAO } from "src/constants";
...
class StudentService {
  ...
  constructor(
    ...
    @Inject(PENALTY_DAO) private penaltyRepo: typeof PenaltyModel
  ) {}
  ...
  async getStudent(id: string, t?: Transaction): Promise<Student> {
    ...
    pr.push(
      this.penaltyRepo.findAll({
        attributes: [[sequelize.fn("SUM", sequelize.col("penalty")), "totalPenalty"]],
        where: {
          studentId: id,
        },
      })
    );
    const [ans, penalty] = await Promise.all(pr);
    const student = ans.get({ plain: true });
    const totalPenalty = penalty[0]?.dataValues?.totalPenalty || 0;
  ...
  async updateStudent(data, t?: Transaction): Promise<Student> {
    ...
    const pr = [];
    pr.push(
      this.studentRepo.update(studentData, {
