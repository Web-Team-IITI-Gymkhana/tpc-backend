      ...
      if (directError.message.includes("is being accessed by other users")) {
        logger.warn("⚠️  Direct restore failed due to active connections, trying alternative method...");
         ...
         for (let i = 0; i < lines.length; i++) {
           ...
           if (trimmedLine.startsWith('ALTER TABLE') && 
           ...
           const shouldFilter = trimmedLine.startsWith('DROP DATABASE') ||
         ...
         if (stderr) {
           ...
           const seriousErrors = allErrors.filter(line => 
           ...
           const createStatements = stderr.split('\n').filter(line => 
             line.includes('CREATE TABLE') || 
             line.includes('CREATE TYPE') ||
             line.includes('CREATE CONSTRAINT')
           ).length;
           
           const totalErrors = allErrors.length;
           const ignorableErrors = totalErrors - seriousErrors.length;
           
           logger.log(`📊 Restore summary: ${createStatements} objects created, ${ignorableErrors} ignorable conflicts, ${seriousErrors.length} serious errors`);
         }
